...
# If a raw token isn't provided, try SSM (preferred path)
data "aws_ssm_parameter" "github_token" {
  count           = var.github_token == null && var.github_token_param != null ? 1 : 0
  name            = var.github_token_param
  with_decryption = true
}

locals {
  github_token_effective = (
    var.github_token != null && length(var.github_token) > 0
    ? var.github_token
    : (var.github_token_param != null
      ? data.aws_ssm_parameter.github_token[0].value
    : null)
  )
}

# Fail fast if we need a token and couldn't resolve one
check "github_token_present" {
  assert {
    condition     = local.github_token_effective != null && length(local.github_token_effective) > 0
    error_message = "CodeBuild: no GitHub token provided. Set github_token OR github_token_param (SSM SecureString)."
  }
}
...
data "aws_iam_policy_document" "inline" {
  ...
  # Allow the script to pull/push private artifacts
  statement {
    sid = "S3PrivateSecretsBucket"
    actions = [
      "s3:GetObject",
      "s3:PutObject",
      "s3:DeleteObject",
    ...
    resources = [
      "arn:aws:s3:::insizon-aws-private",
      "arn:aws:s3:::insizon-aws-private/*"
    ]
  }
}
...
resource "aws_codebuild_source_credential" "github_pat" {
  ...
  server_type = "GITHUB"
  token       = local.github_token_effective
}
